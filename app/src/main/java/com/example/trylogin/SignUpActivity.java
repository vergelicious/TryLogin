...
public class SignUpActivity extends AppCompatActivity {
    ...
    private boolean validateUsername() {
        ...
        if (val.isEmpty()) {
            ...
            username.setError("Username should not be greater than 20 characters!");
            return false;
    ...
    private boolean validatePassword() {
        ...
        String checkPassword = "^(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z])(?=\\S+$).{3,}$";

        if (val.isEmpty()) {
            password.setError("Field can not be empty!");
            return false;
        } else if (!val.matches(checkPassword)) {
            password.setError("Password must contain mix of upper and lower case letters as well as digits.");
            return false;
        } else {
